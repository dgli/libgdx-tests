package com.mygdx.game.sharedworld.screens.glasssimulator.config;

/**
 * Created by dgli on 24/01/15.
 */
public final class Constants {
    public static final float RAY_UNIT_WIDTH = 1.5f;

    public static class LaserPointer{
        public static final float BARREL_LENGTH = 10f;
        public static final float BARREL_DIAMETER = 1f;
        public static final float BASE_DOT_RADIUS = 2f;
    }

    public static class InterfaceSegment {
        public static final float WALL_THICKNESS = 2f;
        public static final float END_POINT_DOT_RADIUS = 2f;
    }
}
